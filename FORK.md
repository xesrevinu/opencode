...
若 12+ 个历史 commit 逐条 rebase 冲突爆炸，可改用 **diff replay**（2026-09-03 做法）：

```sh
git checkout v2
git reset --hard upstream/v2
git diff $(git merge-base upstream/v2 v2-prerebase-backup) v2-prerebase-backup \
  -- . ':!packages/session-viewer' ':!packages/tui/src/routes/session/rows.ts' ':!packages/tui/src/routes/session/rows-reduce.ts' \
  | git apply
# 修冲突、跑测试后单次 commit
...
git reset --hard v2
git cherry-pick <first-session-viewer-commit>^..<last-session-viewer-commit>

# 方式 B：整支 rebase
git rebase v2
```

`rows.ts` 冲突时：取 slim orchestration 版 + 独立 `rows-reduce.ts`，把 upstream 行逻辑 patch 进 reducer。
